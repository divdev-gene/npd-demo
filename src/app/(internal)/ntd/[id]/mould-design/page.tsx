...
function statusLabel(status: ComponentFinalStatus) {
  switch (status) {
    case "approved":
      return (
        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-100">
...
function verdictBadge(verdict: NTDMouldReviewEntry["verdict"]) {
  if (verdict === "approved")
    return (
  ...
  if (verdict === "rejected")
    return (
      <span className="px-1.5 py-0.5 rounded text-[10px] font-semibold bg-red-50 text-red-700 border border-red-100">
        Rejected
      </span>
    )
  return (
...
interface MouldComponentCardProps {
...
function MouldComponentCard({
  component,
  isRnd,
  isSpocOrSourcing,
  currentRole,
  onSaveReview,
}: MouldComponentCardProps) {
  ...
  const isLocked = component.locked && component.final_status === "approved"

  const latestIteration: NTDMouldIteration | undefined =
  ...
  // Determine SLA deadline to show
  const slaDeadline =
    latestIteration?.overall_status === "under_review"
      ? latestIteration.rnd_review.sla_deadline || latestIteration.sourcing_review.sla_deadline
      : ""
  ...
  return (
    <div
      className={[
        ...
        isLocked ? "border-emerald-100 bg-emerald-50/30" : "border-slate-100 bg-white",
      ].join(" ")}
    >
      {/* Row header */}
      <button
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          {slaDeadline && <SLABadge deadline={slaDeadline} />}
          {component.iterations.length > 0 && (
            <span className="text-[10px] text-slate-400">Iter. {component.iterations.length}</span>
          )}
      ...
      {isLocked && (
        <div className="px-4 pb-3 flex items-center gap-2 text-[12px] text-emerald-700">
          ...
          <span>
            Approved — locked after {component.iterations.length} iteration
          ...
          {latestIteration ? (
            <div className="rounded-lg bg-slate-50 border border-slate-200 px-3 py-2.5 space-y-1.5">
              <p className="text-[11px] font-semibold text-slate-500 uppercase tracking-wide">
                Latest Submission — Iteration {latestIteration.iteration_no} (Round{" "}
                {latestIteration.redesign_round})
              </p>
            ...
            <div className="rounded-lg bg-amber-50 border border-amber-100 px-3 py-2.5">
              <p className="text-[12px] text-amber-700">
                No submission yet. Waiting for supplier to submit Mould Design 3D and MFA PPT.
              </p>
            </div>
          )}
              ...
              {sourcingAlreadyReviewed ? (
                ...
                <>
                  ...
                  <button
                    ...
                    disabled={
                      !sourcingVerdict || (sourcingVerdict === "rejected" && !sourcingComments.trim())
                    }
                    className="px-4 py-2 rounded-lg text-[12px] font-semibold bg-blue-600 hover:bg-blue-700 text-white transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                  >
              ...
              {historyExpanded && (
                <div className="mt-2 space-y-2">
                  {[...component.iterations].reverse().map((iter) => (
                    <div
                      key={iter.iteration_no}
                      className="rounded-lg border border-slate-100 bg-slate-50/50 px-3 py-2.5 text-[11px] space-y-1"
                    >
                      <div className="flex items-center justify-between flex-wrap gap-1">
                        <span className="font-semibold text-slate-700">
                          Iteration {iter.iteration_no} (Round {iter.redesign_round})
                        </span>
                        <span
                          className={[
...
export default function MouldDesignPage() {
  ...
  const [mouldData, setMouldData] = useState<NTDMouldDesignData | null>(null)
  ...
  const initMouldDesign = useCallback(() => {
    let data = getNTDMouldDesign(id)

    if (!data) {
      const initiation = getNTDInitiation(id)
      const comps = initiation?.components ?? []
      const initialData: NTDMouldDesignData = {
        redesign_round: 0,
      ...
      setNTDMouldDesign(id, initialData)
      data = initialData
    }

    setMouldData(data)
    setSubstageComplete(data.substage_complete)
  }, [id])
  ...
  function handleSaveReview(
    ...
    const updatedComponents = mouldData.components.map((comp) => {
      ...
      const reviewEntry: NTDMouldReviewEntry = {
        verdict,
        comments,
      ...
      let newFinalStatus: NTDMouldComponent["final_status"] = comp.final_status
    ...
    const updatedData: NTDMouldDesignData = {
      ...mouldData,
      components: updatedComponents,
      substage_complete: allApproved,
    }

    setNTDMouldDesign(id, updatedData)
    setMouldData(updatedData)

    if (allApproved && !substageComplete) {
      setSubstageComplete(true)

      // If this is a redesign round, mark it resolved
      if (mouldData.redesign_round > 0) {
        const redesignData = getNTDRedesign(id)
        if (redesignData) {
          const updatedRounds = redesignData.redesign_rounds.map((r) =>
            r.mould_design_redesign_round === mouldData.redesign_round && r.resolved_at === ""
              ? { ...r, resolved_at: now }
              : r,
          )
          setNTDRedesign(id, { redesign_rounds: updatedRounds })
        }
      }
    }
  }

  const progress = getMouldDesignProgress(id)
  const progressPct = progress.total > 0 ? Math.round((progress.approved / progress.total) * 100) : 0

  // Find redesign info for banner
  const redesignData = mouldData && mouldData.redesign_round > 0 ? getNTDRedesign(id) : null
  const activeRedesignRound = redesignData?.redesign_rounds.find(
    (r) => r.mould_design_redesign_round === mouldData?.redesign_round && r.resolved_at === "",
  )

  if (!mouldData) {
    return (
      <div className="flex items-center justify-center h-full">
        <span className="text-[13px] text-slate-400">Loading Mould Design data…</span>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50">
        ...
        <div className="flex items-start justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-[16px] font-bold text-slate-900">
              Mould Design Review — Stage 8A
            </h1>
            <p className="text-[12px] text-slate-500 mt-0.5">
              Per-component Mould Design + MFA review. Both R&amp;D and Sourcing must approve each
              component. SLA: 24 hours per submission.
            </p>
          </div>
      ...
      <div className="flex-1 overflow-auto px-6 py-6 space-y-3 max-w-4xl mx-auto w-full">
        {/* Redesign banner */}
        {mouldData.redesign_round > 0 && activeRedesignRound && (
          <div className="flex items-start gap-3 px-4 py-3 rounded-xl bg-amber-50 border border-amber-100">
            <AlertCircle className="w-5 h-5 text-amber-600 shrink-0 mt-0.5" />
            <div>
              <p className="text-[13px] font-bold text-amber-800">
                Redesign Round {mouldData.redesign_round} — triggered by Trial{" "}
                {activeRedesignRound.triggered_by_trial}
              </p>
              <p className="text-[11px] text-amber-600">
                {activeRedesignRound.failed_component_ids.length} component
                {activeRedesignRound.failed_component_ids.length !== 1 ? "s" : ""} re-entered
                from that trial. Locked components remain approved.
              </p>
            </div>
          </div>
        )}
          ...
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-emerald-50 border border-emerald-100">
            ...
            <div>
              <p className="text-[13px] font-bold text-emerald-800">
                Mould Design Complete — All components approved.
              </p>
              <p className="text-[11px] text-emerald-600">
                {mouldData.redesign_round === 0
                  ? "Proceeding to Stage 8B: Manufacturing."
                  : "Redesign complete — new trial can now be initiated."}
              </p>
            </div>
          </div>
        )}
        ...
        {!isRnd && !isSpocOrSourcing && (
          <div className="rounded-xl bg-amber-50 border border-amber-100 px-4 py-3 text-[12px] text-amber-700">
            You are viewing as <strong>{currentRole}</strong>. Review actions are available to
            R&amp;D and Sourcing roles.
          </div>
        )}
